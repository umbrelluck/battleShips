...
class Game:
    ...
    def prnt(self, info, end=False):
        if end:
            for i in range(-1, 11):
                tmp = str(info[i]) if info != [1, 2, 3] and i in [0, 1] else ""
            ...
            if self.pc:
                ...
                for i in range(-1, 11):
                    ...
                    print(self.players[1].field.field_without_ships(i, end="             ") +
                          self.players[0].field.field_without_ships(i, end="             ") + tmp)
    ...
    def play(self):
        ...
        # try:
        #     info[2] = "Available shots: " + str(self.players[(self.current_pl + 1) % 2].to_hit)
        # except AttributeError:
        #     info[2] = "Available shots: " + str(self.players[(self.current_pl + 1) % 2].hited)
        while flag:
            # victory
            # info[1] = self.players[self.current_pl].name + "`s ships: " + str(
            #     self.players[(self.current_pl + 1) % 2].field.shp)
            info[1] = self.players[(self.current_pl + 1) % 2].name + "`s ships: " + str(
                self.players[self.current_pl].field.shp)
            # try:
            #     info[2] = "Available shots: " + str(self.players[self.current_pl].to_hit)
            # except AttributeError:
            #     info[2] = "Available shots: " + str(self.players[self.current_pl].hited)
